...
\begin{itemize}
	...
	\begin{itemize}
		...
		\item \textbf{Django REST Framework SimpleJWT} — used to implement secure JWT-based authentication and token management \cite{RFC7519,SimpleJWT2024}.
		
		\item \textbf{django-cors-headers} — used to enable secure communication between the frontend and backend hosted on different origins \cite{OttoKristensen2024CORS}.
		
		\item \textbf{django-filter} — used to simplify API filtering and query parameter handling in DRF endpoints \cite{DjangoFilter2024}.
	\end{itemize}
	...
	\item \textbf{Static File Serving:} \textbf{WhiteNoise} — used to serve compressed static assets efficiently in production \cite{WhiteNoise2024}.
	
	\item \textbf{Environment and Secret Management:} \textbf{python-dotenv} — used to manage sensitive configuration variables through environment files \cite{Wiggins2012TwelveFactor,PythonDotenv2024}.
	
	\item \textbf{TLS Certificate Verification:} \textbf{Certifi} — used to ensure secure HTTPS certificate validation for external services \cite{Certifi2024}.
	
	\item \textbf{AI Inference:} \textbf{Groq API} — integrated to provide AI-powered conversational assistance for farmers \cite{GroqInc2024}.
	
	\item \textbf{Data Visualisation:} \textbf{Chart.js} and \textbf{react-chartjs-2} — used to display interactive analytics and dashboard charts \cite{ChartJS2024,ReactChartJS2024}.
	
	\item \textbf{Interactive Maps:} \textbf{Leaflet} and \textbf{react-leaflet} — used to render geographic information and map-based visualisation features \cite{AgafonkinLeaflet2024,ReactLeaflet2024}.
	
	\item \textbf{Icon System:} \textbf{Lucide React} — used to provide scalable and consistent interface icons across the application \cite{LucideReact2024}.
